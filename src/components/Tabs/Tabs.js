...
export default function BasicTabs({data, labelTabs, uniqueKey}) {
  ...
  return (
    ...
    <div>
      <Carousel uniqueKey={uniqueKey} isSongs={true} albumns={data} />
    </div>
  </CustomTabPanel>
      {data && labelTabs.map((ele, idx) => (
  <CustomTabPanel key={idx} value={value} index={idx+1}>
    <div>
      <Carousel uniqueKey={uniqueKey} isSongs={true} albumns={data.filter((element) => element.genre.key === ele.key)} />
    </div>
  </CustomTabPanel>
))}
